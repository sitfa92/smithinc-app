...
// ── WhatsApp conversation memory (in-memory, keyed by sender phone number) ──
const WA_CONVERSATIONS = new Map();
const WA_CONVO_TTL_MS = 30 * 60 * 1000; // reset after 30 min of inactivity
const WA_MAX_HISTORY = 10;               // keep last 10 user+assistant pairs
...
function twimlSayAndListen({ message, actionPath = "/gather", first = false }) {
  ...
  const intro = first
    ? `<Say voice=\"Polly.Joanna\">Hi, this is ${escapeXml(BOT_NAME)}. How can I help you today?</Say>`
    : "";

  return `<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n<Response>\n  ${intro}\n  ${message ? `<Say voice=\"Polly.Joanna\">${escapeXml(message)}</Say>` : ""}\n  <Gather input=\"speech\" speechTimeout=\"auto\" timeout=\"4\" action=\"${escapeXml(actionUrl)}\" method=\"POST\">\n    <Say voice=\"Polly.Joanna\">You can speak now.</Say>\n  </Gather>\n  <Say voice=\"Polly.Joanna\">I did not catch that. Goodbye.</Say>\n  <Hangup/>\n</Response>`;
}
...
app.post("/gather", async (req, res) => {
  ...
  app.post("/whatsapp", async (req, res) => {
    ...
    // Empty or media-only message (no text body)
    if (!incoming) {
      res.type("text/xml").status(200).send(
    ...
    // Stop / opt-out words — Twilio handles STOP automatically, but be safe
